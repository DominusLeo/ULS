...
char			*ft_qstr(char *s, int *i)
{
	...
	j = -1;
	if (i[11] < 0 || i[11] > (int)mx_strlen(s))
		res = mx_strdup(s);
	else
	{
	...
	i[10] -= (int)mx_strlen(res) - 1;
	if ((i[3] != 2 && (i[13] = 1)) || i[8] == 9)
		strendf(res[0], i);
	if (res[i[13]] && res[0])
		i[1] += write(1, &res[i[13]], mx_strlen(&res[i[13]]));
	while ((i[10] -= 1) > 0)
		i[1] += write(1, " ", 1);
	free(res);
	return (s);
...
static	char	*ft_rotng(char *s, int i)
{
	...
	if (!*s)
		return (0);
	i = -i;
	if (!(r = (char *)malloc(mx_strlen(s) + 1)))
		return (0);
	res = r;
	while (*s)
	{
...
static	void	outunsec(char *s, int *i, int j, int c)
{
	...
	s[c] = '\0';
	if (i[10] > (int)mx_strlen(s))
	{
		while (i[3] == 2 && i[10] > (int)mx_strlen(s))
			s = ft_joinfree(s, " ", 1);
		while (i[3] == 1 && i[10] > (int)mx_strlen(s))
			s = ft_joinfree("0", s, 0);
		while (i[10] > (int)mx_strlen(s))
			s = ft_joinfree(" ", s, 0);
	}
