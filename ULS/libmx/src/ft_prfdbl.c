...
static int		justfree(char **ito, int a, char c)
{
	if (!ito)
	{
		if (c == 'G' || c == 'E')
...
static	char	*enddbl(char *s, int *i, int r, int l)
{
	...
	if (i[3] == 2 && i[2] == 2 && *s != '-' && (i[1] += write(1, "+", 1)))
		i[1] += write(1, &s[i[14]], mx_strlen(&s[i[14]]));
	else if (i[3] == 2)
		i[1] += write(1, &s[i[14]], mx_strlen(&s[i[14]]));
...
static	int		dbltoa(long double c, int i, char **s, long double y)
{
	...
	if (mx_strlen(s[0]) + mx_strlen(s[1]) > 8)
		return (0);
	return (1);
}
...
void			ft_precforp(char *s, int *i)
{
	...
	{
		while (i[3] == 1 && i[10] - 2 > (int)mx_strlen(s))
			s = ft_joinfree("0", s, 2);
		while (i[11] > (int)mx_strlen(s))
			s = ft_joinfree("0", s, 2);
		s = ft_joinfree("0x", s, 2);
	}
	while (i[3] == 2 && i[10] > (int)mx_strlen(s))
		s = ft_joinfree(s, " ", 1);
	while (i[3] != 2 && i[10] > (int)mx_strlen(s))
		s = ft_joinfree(" ", s, 2);
	i[1] += write(1, s, mx_strlen(s));
	free(s);
}
...
void			ft_prfdbl(char c, int *i, va_list *ar)
{
	...
	enddbl(ito[0], i, mx_strlen(ito[0]) + mx_strlen(ito[1]) + 1, l);
	if (i[11] && (i[1] += write(1, ".", 1)))
		i[1] += write(1, &ito[1][1], mx_strlen(&ito[1][1]));
	if (l != -1 && (i[1] += 4))
		justfree(0, l, c);
	justfree(ito, 2, 0);
}
