...
static	char	*checkshar(char *ito, int *i, char c)
{
	...
	{
		i[10] -= 2;
		if (!(res = (char *)malloc(sizeof(char) * mx_strlen(ito) + 3)))
			return (0);
		res[0] = '0';
		res[1] = c;
		res[2] = '\0';
...
static	void	prfsec(char c, int *i, va_list *ar, char *ito)
{
	...
	if (i[10] > (int)mx_strlen(ito) || i[11] > -1 || i[4])
		ito = endprf(ito, i, mx_strlen(ito), c);
	else if (i[2] == 2 && (i[1] += write(1, "+", 1)))
		i[1] += write(1, ito, mx_strlen(ito));
...
void			ft_prfnbr(char c, int *i, va_list *ar, char *ito)
{
	if ((c == 'd' || c == 'i' || c == 'D') && (i[13] = 1))
	{
		...
		if (i[10] > (int)mx_strlen(ito) || i[11] > -1)
			ito = endprf(ito, i, mx_strlen(ito), c);
